...
<body>
<!--POV invoice start-->
<div class="order-slip-pov" style="min-width: 333px">
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="receipt__detail">
                    <div class="title" style="text-align: center; font-weight: bold; font-size: 20px;"><span>{{env('APP_NAME')}}</span></div>


                    <div class="company_detail" style="margin-bottom: -15px !important; text-align: center; font-weight: bold; font-size: 15px;">
                        ...
                        @if(!is_null(getPanNumber()))
                            <p style="margin-bottom: -5px">{{'PAN NO. '. getPanNumber()}}</p>
                        @endif
                        ...
                        <table class="table">
                            ...
                            @forelse($order->orderProducts as $product)
                                <tr class="b-bottom">
                                    <td style="text-align: left;" class="item-detail">
                                        {{Str::limit(ucwords($product->title), 20)}}
                                    </td>
                            ...
                            @if($order->orderProducts->isNotEmpty())
                                <tr class="text-right">
                                    ...
                                    <td colspan="2" class=""
                                        style="text-align:right; color:#030303; text-transform:uppercase;">
                                ...
                                @if(!empty($order->detail['couponDiscount']) && $order->detail['couponDiscount'] > 0)
                                    <tr class="text-right">
                                        ...
                                        <td colspan="2" class=""
                                            style="text-align:right; color:#000000; text-transform:uppercase;">
                                ...
                                @php
                                    ...
                                    if (!empty($order->detail['couponDiscount']) && $order->detail['couponDiscount'] > 0) {
                                        $subTotal = $subTotal - $order->detail['couponDiscount'];
                                    }
                                    if (!empty($order->detail['discount']) && $order->detail['discount'] > 0) {
                                        $subTotal = $subTotal - $order->detail['discount'];
                                    }
                                @endphp
                                    ...
                                    @if(!empty($order->detail['subTotal']) && $order->detail['subTotal'] > 0)
                                        <tr class="text-right">
                                            <td colspan="3" style="text-align:right;">
                                                ...
                                                <span style="padding-left:10px;">:</span></td>
                                            <td colspan="2" class=""
                                                style="text-align:right; color:#000000FF; text-transform:uppercase;">
                                                {{number_format($subTotal/getDefaultCurrecyRate(), 2)}}
                                            </td>
                                        </tr>
                                    @endif
                                @endif
                                ...
                                <tr class="text-right">
                                    ...
                                    <td colspan="2" class=""
                                        ...
                                        {{getDefaultCurrencyTitle()}}{{round($order->total/getDefaultCurrecyRate())}}
                                    </td>
                                </tr>
                                <tr class="grand-total-border">
                                    <td colspan="7" style=" padding-bottom:10px;">
                                ...
                                <tr>
                                    <td colspan="7"
                                        style="letter-spacing:1px;text-align:center;">{{getDefaultCurrencyTitle().ucwords(convertDigitToText()->format(round($order->total/getDefaultCurrecyRate())))}} {{translate('Only')}}
                                    </td>
                                </tr>
                                <tr class="grand-total-border">
                                    <td colspan="7" style=" padding-bottom:10px;">
